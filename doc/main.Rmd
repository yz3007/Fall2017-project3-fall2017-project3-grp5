...
label_test <- labels[sampling,-1]
label_train <- labels[-sampling,-1]
dat_test <- sift_data[sampling,-1]
dat_train <- sift_data[-sampling,-1]
```
...
The best parameter of depth is 11.
...
We save the fitted trainning model out in case of using it in the near future.
...
We save the prediction values out based on my sampling test data as well as using fitted trainning model.
...
By compring our prediction with true value, we get around 75% correction rate whihc is not too bad.
...
Overall all, the time comsuming of training model is around 40 minutes.
The time for making prediction takes less than 1 second.